...
def check_params(q, redundancy, stages):
    if q < 1:
...
def tqwt(x, q, redundancy, stages):
    """
    ...
    for subband_idx in range(1, stages + 1):
        n0 = int(2 * round(alpha ** subband_idx * n / 2))
        n1 = int(2 * round(beta * alpha ** (subband_idx - 1) * n / 2))
        # print("alpha, beta is: ", alpha, beta)
        # print("n0, n1 is: ", n0, n1)
        fft_of_x, w_subband = analysis_filter_bank(fft_of_x, n0, n1)
        candi = ifft(w_subband) * np.sqrt(len(w_subband))
        w.append(candi)  # inverse unitary DFT

    candi = ifft(fft_of_x) * np.sqrt(len(fft_of_x))
    w.append(candi)    # inverse unitary DFT
...
def analysis_filter_bank(x, n0, n1):
    ...
    """
    x = np.array([val.real for val in x])
    n = x.shape[0]                  # len(x)
