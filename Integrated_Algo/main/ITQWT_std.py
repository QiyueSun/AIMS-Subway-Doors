...
def itqwt(w, q, redundancy, n):
    ...
    # print(type(w[num_subbands-1]))
    w[num_subbands-1] = np.array([val.real for val in w[num_subbands-1]])
    # print(fft(w[num_subbands-1]))
    # print(np.sqrt(w[num_subbands-1].shape[0]))
...
def synthesis_filter_bank(lp_subband, hp_subband, n):
    """
